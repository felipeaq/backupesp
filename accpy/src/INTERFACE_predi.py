...
class Ui_MainWindow(object):
    def setupUi(self, MainWindow, screen_size):
        ...
        self.prob_correct_value = QtWidgets.QLabel(self.lateralMenuLayoutWidget)
    ...
    def _update_canvas(self):
        ...
        t1, xfft = ReadRoutine().sensors.list_s[2].a[0].getxyFFT(
            ReadRoutine().sensors.rtc)
        t2, yfft = ReadRoutine().sensors.list_s[2].a[1].getxyFFT(
            ReadRoutine().sensors.rtc)
        t3, zfft = ReadRoutine().sensors.list_s[2].a[2].getxyFFT(
            ReadRoutine().sensors.rtc)
        t4, x = ReadRoutine().sensors.list_s[2].a[0].getxy(
            ReadRoutine().sensors.rtc)
        t5, y = ReadRoutine().sensors.list_s[2].a[1].getxy(
            ReadRoutine().sensors.rtc)
        t6, z = ReadRoutine().sensors.list_s[2].a[2].getxy(
            ReadRoutine().sensors.rtc)
        ...
        KPredictior().append_predict(ReadRoutine().sensors.list_s[2].a[2].get_last_fft())
        self.probCorrect()
        self.probunbalance()
        self.probFriction()
    ...
    def probCorrect(self):
        ...
        self.prob_correct_value.setText(str(round(KPredictior().get_prob()[0]*100, 2))+"%")

    def probunbalance(self):
        self.prob_unbalance_value.setStyleSheet("background-color: rgb(0, 255, 0);\n"
                                    "color: rgb(0, 0, 0);")
        self.prob_unbalance_value.setText(str(round(KPredictior().get_prob()[1]*100, 2))+"%")

    def probFriction(self):
        self.prob_friction_value.setStyleSheet("background-color: rgb(0, 255, 0);\n"
                                    "color: rgb(0, 0, 0);")
        self.prob_friction_value.setText(str(round(KPredictior().get_prob()[2]*100, 2))+"%")
