...
class Ui_MainWindow(object):
    ...
    def _update_canvas(self):
        ...
        t1, xfft = ReadRoutine().sensors[0].getXY("gyroRad0")
        t2, yfft = ReadRoutine().sensors[0].getXY("gyroRad1")
        t3, zfft = ReadRoutine().sensors[0].getXY("gyroRad2")
        t4, x = ReadRoutine().sensors[0].getXY("accGravity0")
        t5, y = ReadRoutine().sensors[0].getXY("accGravity1")
        t6, z = ReadRoutine().sensors[0].getXY("accGravity2")
        t7, magx = ReadRoutine().sensors[0].getXY("magNorm0")
        t8, magy = ReadRoutine().sensors[0].getXY("magNorm1")
        t9, magz = ReadRoutine().sensors[0].getXY("magNorm2")
        t10, pitch = ReadRoutine().sensors[0].getXY("pitch")
        t11, roll = ReadRoutine().sensors[0].getXY("roll")
        t12, yaw = ReadRoutine().sensors[0].getXY("yaw")

        # print(np.diff(t6))
        # print("mean={}, std={}".format(np.mean(np.diff(t6)), np.std(np.diff(t6))))
        if len(t4) != 0:
            if t4[-1] >= self.gap[1]:
                self.gap[0] = int(ReadRoutine().sensors[0].rtc[0])
                self.gap[1] = int(ReadRoutine().sensors[0].rtc[-1])
