...
class charts:
    ...
    def start_chart(self):
        ...
        for j in range(3):
            ...
            for i in range(Sensors.TOTAL_S):
                ...
                self.lg[j].append(l)
            self.ax[0][j].relim()
            self.ax[0][j].set_ylim(-(2**15/Sensors.TO_RAD*1.1),
                                   (2**15/Sensors.TO_RAD*1.1))
        ...
        for j in range(3):
            ...
            for i in range(Sensors.TOTAL_S):
                ...
                self.la[j].append(l)
            self.ax[1][j].relim()
            self.ax[1][j].set_ylim(-2**15/Sensors.RESIST *
                                   1.1, 2**15/Sensors.RESIST*1.1)
    ...
    def update_chart(self):
        while len(ReadRoutine().sensors[0].rtc) < 2:
            time.sleep(0.01)
        ...
        while True:
            ...
            try:
                self.__update_chart()
                if len(ReadRoutine().sensors[0].rtc) > 0:
                    self.fig.canvas.draw()
                plt.pause(0.03)
        ...
        for j in range(3):
            for i in range(Sensors.TOTAL_S):
                if ReadRoutine().active_sensors[i]:
                    x, y = ReadRoutine().sensors[0].list_s[i].a[j].getxy(
                        ReadRoutine().sensors[0].rtc)
                    self.lg[j][i].set_xdata(x)
                    self.lg[j][i].set_ydata(y)
            self.ax[0][j].set_xlim(
                ReadRoutine().sensors[0].rtc[0], ReadRoutine().sensors[0].rtc[-1])
